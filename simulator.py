...
class State:
    ...
    def printState(self):
        ...
        with open(outputFileName + "_sim.txt", 'a') as outFile:
            ...
            outFile.write(outStr + "\n")
            outFile.write("\ndata:\n")
            while len(self.dataval) % 8 != 0 and len(self.dataval) != 0:
            ...
            if self.opcode[i] == 0:  # NOP
                ...
                '''
                    if address[-1] <= target address
                        ...
                        good to store/load
                '''
                target = 152  # need to calculate this to fix it. r4, #1 -> under r3
                ...
                target = 11  # index of value pointed to by [R4, #1] (dataval[10] <==> data 152 <==> int 11

                armState.R[self.arg1[i]] = armState.dataval[target]
